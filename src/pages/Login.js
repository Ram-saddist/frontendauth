...
export default function Login() {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        axios.post('http://13.234.238.195:5000/api/auth/login', login).then(res => { localStorage.setItem('token', res.data.token); navigate('/') }).catch(err => alert(err.response.data));
    }
    return (
        <div className='login'>
